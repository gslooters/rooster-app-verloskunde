...
BEGIN
    ...
    IF (TG_OP = 'UPDATE') THEN
        ...
        IF (OLD.service_id IS DISTINCT FROM NEW.service_id) THEN
            ...
            -- STAP 2: Blokkeer nieuwe dienst (als die bestaat)
            IF NEW.service_id IS NOT NULL AND NEW.status = 1 THEN
                FOR v_blocked_rec IN 
                    SELECT block_date, block_dagdeel 
                    FROM get_blocked_dagdelen_info(
                        NEW.service_id,
...
DO $$
BEGIN
    RAISE NOTICE '✅ DRAAD 99A: Trigger trg_roster_assignment_auto_blocking SUCCESVOL aangemaakt!';
