...
function formatPeriodDDMM(isoStart: string, daysCount = 35) {
  const d0 = toDate(isoStart);
  const de = toDate(addDaysISO(isoStart, daysCount - 1));
...
function dayShort(iso: string) {
  ...
  return map[toDate(iso).getDay()];
}
function isWeekend(iso: string) {
  const s = dayShort(iso);
  return s === 'ZA' || s === 'ZO';
}
...
export default function PlanningGrid({ rosterId }: { rosterId: string }) {
  ...
  function setService(date: string, empId: string, service: string) {
    setCells((prev) => ({
      ...prev,
      [date]: { ...prev[date], [empId]: { ...prev[date][empId], service } },
    }));
  }
  ...
  function toggleLock(date: string, empId: string) {
    setCells((prev) => ({
      ...prev,
      [date]: { ...prev[date], [empId]: { ...prev[date][empId], locked: !prev[date][empId].locked } },
    }));
  }
  ...
  return (
    <main className="p-4">
      ...
      <div className="flex items-center justify-between mb-3">
        <h1 className="text-xl font-semibold">Periode: {formatPeriodDDMM(start)}</h1>
        <div className="flex items-center gap-2">
          {isDraft && (
            <button className="px-3 py-1.5 text-sm border rounded" onClick={() => setShowAvail((s) => !s)}>
              {showAvail ? 'Sluit Beschikbaarheid' : 'Beschikbaarheid'}
            </button>
          )}
        </div>
      </div>
      ...
      {showAvail && isDraft && (
        <div className="mb-3">
          <AvailabilityPanel rosterId={roster.id} employees={EMPLOYEES} days={days} />
        </div>
      )}

      <div className="overflow-auto max-h-[70vh] border rounded">
        <table className="min-w-[1100px] border-separate border-spacing-0 text-[12px]">
          <thead>
            <tr>
              <th className="sticky left-0 top-0 z-30 bg-gray-50 border px-2 py-1 align-top w-[120px]" rowSpan={2}>
                Medewerker
              </th>
              {weekGroups.map((g) => (
                <th
                  key={`w-${g.week}-${g.startIndex}`}
                  className="sticky top-0 z-20 bg-gray-100 border px-2 py-1 text-[11px] text-gray-800"
                  colSpan={g.span}
                >
                  Week {g.week}
                </th>
