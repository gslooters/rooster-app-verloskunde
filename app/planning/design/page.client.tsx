...
function normalizeTeam(input: unknown): 'Groen' | 'Oranje' | 'Overig' {
  const raw = extractTeamRaw(input).normalize('NFKD').replace(/[\u0300-\u036f]/g,'');
...
export default function DesignPageClient() {
  ...
  // Canonical sortering met normalizeTeam
  const sortedEmployees = useMemo(() => {
    const order: Record<'Groen'|'Oranje'|'Overig', number> = { Groen: 0, Oranje: 1, Overig: 2 };
    return [...employees].sort((a, b) => {
      const aC = normalizeTeam((a as any).team);
      const bC = normalizeTeam((b as any).team);
      if (aC !== bC) return (order[aC] ?? 3) - (order[bC] ?? 3);
  ...
  const computedValues = useMemo(() => {
    if (!designData) {
      return { startISO: new Date().toISOString().split('T')[0], startDate: new Date(), endDate: new Date(), weeks: [], periodTitle: '', dateSubtitle: '' };
    }
  ...
  useEffect(() => {
    if (employees.length) {
      console.log('TEAM DEBUG (canonical + synced):', employees.map(emp => ({
        ...
        raw: (emp as any).team,
        extracted: extractTeamRaw((emp as any).team),
        normalized: normalizeTeam((emp as any).team),
