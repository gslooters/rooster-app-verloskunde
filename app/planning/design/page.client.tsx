...
export default function DesignPageClient() {
  ...
  function updateMaxShiftsHandler(empId: string, maxShifts: number) {
    ...
    const updated = loadRosterDesignData(rosterId); if (updated) { setDesignData(updated); setEmployees(updated.employees); }
  }
  ...
  const weeks = Array.from({ length: 5 }, (_, i) => {
    ...
    return { number: weekNumber, dates: Array.from({ length: 7 }, (_, d) => { const date = new Date(weekStart); date.setDate(weekStart.getDate() + d); return date.toISOString().split('T')[0]; })};
  });
  ...
  // --- SORTERING: Team (Groen->Oranje->Overig), daarna Voornaam ---
              ...
              {['day','date','month'].map((rowType) => (
                <tr key={rowType}>
                  ...
                  {weeks.map(week => week.dates.map(date => {
                    const { day, date: dd, month, isWeekend, isHoliday, holidayName } = formatDateCell(date);
                    ...
                    return (
                      <th key={`${rowType}-${date}`} className={`border-b px-1 py-1 text-xs ${rowType==='day'?'font-medium text-gray-700':rowType==='date'?'text-gray-600':'text-gray-500'} min-w-[50px] ${headerClass}${columnClasses(date)} ${rowType==='day'?'relative':''}`} title={rowType==='date' ? (holidayName || undefined) : undefined}>
