...
export default function DesignPageClient() {
  ...
  const handleAdvanceToEditing = () => {
    ...
    const success = updateRosterDesignStatus(rosterId, { phase: 'bewerking', designComplete: true });
    if (success) {
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex gap-3">
              <button onClick={() => window.location.href = '/planning/design'} className="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">Terug naar overzicht</button>
              <button onClick={handleAdvanceToEditing} className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700">Door naar Bewerking →</button>
            </div>
          </div>

          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            <h3 className="font-semibold text-blue-900 mb-2">Ontwerpfase</h3>
            <p className="text-blue-800 text-sm mb-2">In deze fase stelt u de basisgegevens voor het rooster in. Diensten worden nog niet handmatig ingepland.</p>
          ...
          <div className="overflow-auto border rounded-lg">
            <table className="min-w-full border-separate border-spacing-0 text-sm">
              <thead>
                <tr>
                  <th className="sticky left-0 top-0 z-30 bg-gray-50 border px-3 py-2 text-left w-40" rowSpan={2}>Medewerker</th>
                  <th className="sticky left-40 top-0 z-30 bg-gray-50 border px-2 py-2 text-center w-20" rowSpan={2}>Max<br/>Diensten</th>
                  {weekGroups.map(g => (
                    <th key={`w-${g.week}-${g.startIndex}`} className="sticky top-0 z-20 bg-gray-100 border px-2 py-1 text-center text-xs" colSpan={g.span}>Week {g.week}</th>
                  ))}
                </tr>
                <tr>
                  {days.map(d => {
                ...
                {designData.employees.map(emp => (
                  <tr key={emp.id} className="hover:bg-gray-50">
                    ...
                    <td className="sticky left-40 z-10 bg-white border px-2 py-2">
                      <input type="number" min="0" max="35" value={emp.maxShifts} onChange={(e) => handleMaxShiftsChange(emp.id, e.target.value)} className={`shifts-input w-full text-center ${maxShiftsErrors[emp.id] ? 'border-red-500 bg-red-50' : 'border-gray-300'}`} title="Aantal diensten voor deze periode (0-35)" />
                      {maxShiftsErrors[emp.id] && (<div className="text-xs text-red-600 mt-1">{maxShiftsErrors[emp.id]}</div>)}
                    </td>
                    {days.map(d => {
                      const isUnavailable = isEmployeeUnavailable(rosterId!, emp.id, d);
                      return (
                        <td key={d} className="border p-1 text-center">
                          <button onClick={() => handleToggleUnavailability(emp.id, d)} className={`w-8 h-8 rounded text-xs font-bold transition-colors ${isUnavailable ? 'not-available bg-red-100 border border-red-300 hover:bg-red-200' : 'bg-white border border-gray-200 hover:bg-gray-100'}`} title={isUnavailable ? 'Niet beschikbaar - klik om beschikbaar te maken' : 'Beschikbaar - klik om niet beschikbaar te maken'}>
                            {isUnavailable ? 'NB' : ''}
                          </button>
                        </td>
                      );
          ...
          <div className="mt-6 flex items-center justify-between text-sm text-gray-600">
            <div>Laatst gewijzigd: {new Date(designData.updated_at).toLocaleString('nl-NL')}</div>
            <div>Status: <span className="font-medium text-blue-600">Ontwerp</span></div>
          </div>
        </div>
      </div>
    </div>
