...
export default function DesignPageClient() {
  ...
  // Verbeterde sortering: team > dienstverband > naam
  ...
  useEffect(() => {
    if (employees.length) {
      console.log('🔍 ROSTER DISPLAY SORTING:', employees.map(emp => ({
        name: (emp as any).voornaam || (emp as any).name,
        team: `${(emp as any).team} → ${normalizeTeam((emp as any).team)}`,
        dienstverband: `${(emp as any).dienstverband} → ${normalizeDienstverband((emp as any).dienstverband)}`
      })));
    }
  }, [employees]);
