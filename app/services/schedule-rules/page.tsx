...
export default function ServicesByDayTypePage() {
  ...
  const updateStaffingRule = (dienstId: string, dagSoort: number, field: 'minBezetting' | 'maxBezetting', value: number) => {
    ...
    if (field === 'minBezetting') {
      newMin = value; if (newMax < newMin) newMax = newMin;
    } else {
      newMax = value; if (newMin > newMax) newMin = newMax;
    }

    const updatedRules = staffingRules.map(rule => (
      rule.dienstId === dienstId && rule.dagSoort === dagSoort
        ? { ...rule, minBezetting: newMin, maxBezetting: newMax, updated_at: new Date().toISOString() }
        : rule
    ));

    const exists = staffingRules.some(r => r.dienstId === dienstId && r.dagSoort === dagSoort);
    if (!exists) {
      updatedRules.push({
        id: `temp_${Date.now()}`,
        dienstId,
        ...
        <div className="bg-white rounded-xl shadow-lg">
          <div className="p-6 md:p-8 border-b border-gray-200">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex gap-2">
                <button onClick={handleResetToDefaults} className="flex items-center gap-2 px-4 py-2 text-gray-600 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors" title="Reset naar standaardwaarden">
                  <RotateCcw className="w-4 h-4" />
                  Reset
                </button>
                <button onClick={() => downloadCSV(staffingRules, services)} className="flex items-center gap-2 px-4 py-2 text-green-600 bg-green-50 rounded-lg hover:bg-green-100 transition-colors" title="Exporteer naar Excel">
                  <FileSpreadsheet className="w-4 h-4" />
                  Excel
                </button>
                <button onClick={() => printToPDF(staffingRules, services)} className="flex items-center gap-2 px-4 py-2 text-red-600 bg-red-50 rounded-lg hover:bg-red-100 transition-colors" title="Exporteer naar PDF">
                  <Download className="w-4 h-4" />
                  PDF
                </button>
                <button onClick={saveAllRules} disabled={!hasChanges || saveStatus === 'saving'} className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-colors ${hasChanges && saveStatus !== 'saving' ? 'text-white bg-blue-600 hover:bg-blue-700' : 'text-gray-400 bg-gray-100 cursor-not-allowed'}`}>
          ...
          <div className="p-6">
            {services.length === 0 ? (
              <div className="text-center py-12">
                ...
                <Link href="/services/types" className="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">Diensten beheren</Link>
              </div>
            ) : (
          ...
          <div className="p-6 bg-gray-50 border-t border-gray-200">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="bg-white p-4 rounded-lg border"><h3 className="font-semibold text-gray-900 mb-2">Bezettingsregels</h3><div className="text-2xl font-bold text-green-600">{staffingRules.length}</div></div>
              <div className="bg-white p-4 rounded-lg border"><h3 className="font-semibold text-gray-900 mb-2">Validatiefouten</h3><div className={`text-2xl font-bold ${validationErrors.size > 0 ? 'text-red-600' : 'text-green-600'}`}>{validationErrors.size}</div></div>
              <div className="bg-white p-4 rounded-lg border"><h3 className="font-semibold text-gray-900 mb-2">Status</h3><div className={`text-sm font-medium px-3 py-1 rounded-full inline-block ${hasChanges ? 'bg-orange-100 text-orange-800' : 'bg-green-100 text-green-800'}`}>{hasChanges ? 'Niet opgeslagen' : 'Opgeslagen'}</div></div>
            </div>
          </div>
        </div>
        {hasChanges && (
