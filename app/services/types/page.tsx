...
export default function ServiceTypesPage() {
  ...
  const openModal = (dienst?: Dienst) => {
    if (dienst) {
      ...
      setFormData({ 
        ...
        planregels: (dienst as any).planregels || ''
      });
    } else {
    ...
    setError(''); setValidationWarnings(''); setShowModal(true);
  };

  const closeModal = () => { setShowModal(false); setEditingDienst(null); setError(''); setValidationWarnings(''); };
  ...
  const handleTimeChange = () => { validateTimes(formData.begintijd, formData.eindtijd); };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); setError('');
    try {
      if (editingDienst) updateService(editingDienst.id, formData as any);
      else createService(formData as any);
  ...
  const generateTimeBar = (begintijd: string, eindtijd: string) => {
    if (begintijd === '00:00' && eindtijd === '00:00') {
      return <div className="w-full h-2 bg-gray-200 rounded-full"><div className="h-full w-0 bg-blue-500 rounded-full"></div></div>;
    }
  ...
  const exportToPDF = () => {
    const printContent = `<!DOCTYPE html><html><head><title>Diensten Overzicht</title><style>
      ...
      h1 { color: #1a365d; } .service { margin-bottom: 20px; padding: 15px; border: 1px solid #e2e8f0; border-radius: 8px; }
      ...
      .service-time { color: #4a5568; margin: 4px 0; } .service-description { color: #718096; font-size: 14px; margin: 4px 0; }
      .system-service { background-color: #f7fafc; } .field-label { font-weight: bold; }
    </style></head><body><h1>Diensten Overzicht</h1><p>Gegenereerd op: ${new Date().toLocaleDateString('nl-NL')}</p>${diensten.map(dienst => `
      <div class="service ${dienst.system ? 'system-service' : ''}">
        <div class="service-header">
          <div class="color-box" style="background-color: ${dienst.kleur}"></div>
          ${dienst.code} - ${dienst.naam} ${dienst.system ? '(Systeemdienst)' : ''} ${dienst.actief ? '' : '(Inactief)'}
        </div>
        ...
        <div class="service-description"><span class="field-label">Beschrijving:</span> ${dienst.beschrijving}</div>
        <div class="service-description"><span class="field-label">Planregels:</span> ${(dienst as any).planregels || 'geen'}</div>
      </div>`).join('')}</body></html>`;
    const newWindow = window.open('', '_blank');
    if (newWindow) { newWindow.document.write(printContent); newWindow.document.close(); newWindow.print(); }
  };

  if (isLoading) {
    return <div className="min-h-screen bg-gradient-to-br from-blue-50 to-green-50 flex items-center justify-center"><div className="text-center"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto mb-4"></div><p className="text-gray-600">Diensten laden...</p></div></div>;
  }
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              ...
              <div className="flex gap-3">
                <button onClick={exportToPDF} className="inline-flex items-center px-4 py-2 bg-gray-600 text-white font-medium rounded-lg hover:bg-gray-700 transition-colors">
                  <span className="mr-2">📄</span>PDF Export
                </button>
                <button onClick={() => openModal()} className="inline-flex items-center px-6 py-2 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 transition-colors">
              ...
              return (
                <div key={dienst.id} className="bg-white border-2 border-gray-200 rounded-xl p-4 hover:shadow-lg transition-all duration-200 relative">
                  <div className="flex items-start gap-3 mb-3">
                    <div className="w-12 h-12 rounded-lg flex items-center justify-center text-white font-bold text-sm" style={{ backgroundColor: dienst.kleur }}>
                      {dienst.code.toUpperCase()}
                    </div>
                    <div className="flex-1">
                      <h3 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
                        {dienst.naam}
                        {dienst.system && <span className="px-2 py-0.5 rounded bg-gray-200 text-gray-700 text-xs font-medium">SYSTEEM</span>}
                      </h3>
                  ...
                  <div className="bg-blue-50 rounded-lg p-3 mb-3">
                    <div className="flex justify-between items-center mb-2">
                      <span className="font-semibold text-gray-900">{dienst.begintijd} - {dienst.eindtijd}</span>
                      <span className="text-sm bg-blue-200 text-blue-800 px-2 py-1 rounded">{dienst.duur}u</span>
                    </div>
                  ...
                  <div className="flex gap-2 mt-auto">
                    {!dienst.system ? (
                      ...
                      <div className="w-full text-center text-xs text-gray-400 py-2 border border-gray-200 rounded">Systeemdienst - Niet bewerkbaar</div>
                    )}
                  </div>
                </div>
              );
                ...
                <form onSubmit={handleSubmit}>
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-3">
                      <div><label className="block text-sm font-medium text-gray-700 mb-2">Code (2-3 tekens)</label>
                        <input type="text" value={formData.code} onChange={(e) => setFormData({...formData, code: e.target.value})} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="bijv. D, ND" maxLength={4} required disabled={editingDienst?.system}/>
                      </div>
                      <div><label className="block text-sm font-medium text-gray-700 mb-2">Naam</label>
                        <input type="text" value={formData.naam} onChange={(e) => setFormData({...formData, naam: e.target.value})} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="bijv. Dagdienst" required disabled={editingDienst?.system}/>
                      </div>
                    </div>

                    <div><label className="block text-sm font-medium text-gray-700 mb-2">Beschrijving</label>
                      <textarea value={formData.beschrijving} onChange={(e) => setFormData({...formData, beschrijving: e.target.value})} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Uitleg van de dienst" rows={2} disabled={editingDienst?.system}/>
                    </div>

                    <div><label className="block text-sm font-medium text-gray-700 mb-2">Planregels</label>
                      <textarea value={formData.planregels} onChange={(e) => setFormData({...formData, planregels: e.target.value})} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Bijv: na deze dienst altijd een Uitnacht inplannen" rows={2} disabled={editingDienst?.system}/>
                    </div>

                    <div className="grid grid-cols-2 gap-3">
                      <div><label className="block text-sm font-medium text-gray-700 mb-2">Begintijd</label>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div><label className="block text-sm font-medium text-gray-700 mb-2">Kleur</label>
                        <div className="flex items-center gap-3">
                          <input type="color" value={formData.kleur} onChange={(e) => setFormData({...formData, kleur: e.target.value})} className="w-12 h-12 rounded-lg border border-gray-300" disabled={editingDienst?.system}/>
                          <div className="w-12 h-12 rounded-lg border border-gray-300 flex items-center justify-center text-white font-bold text-sm" style={{ backgroundColor: formData.kleur }}>{formData.code.toUpperCase() || 'XX'}</div>
                        </div>
                      </div>
                      <div><label className="block text-sm font-medium text-gray-700 mb-2">Dienstwaarde (0–6)</label>
                        <input type="number" min={0} max={6} step={0.5} value={formData.dienstwaarde} onChange={(e) => setFormData({...formData, dienstwaarde: parseFloat(e.target.value) || 0})} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required disabled={editingDienst?.system}/>
                      </div>
                    </div>
                  ...
                  {validationWarnings && <div className="mt-4 p-3 bg-yellow-100 border border-yellow-200 text-yellow-700 rounded-lg text-sm">{validationWarnings}</div>}
                  ...
                  <div className="flex gap-3 mt-6">
                    ...
                    <button type="submit" className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors" disabled={editingDienst?.system}>{editingDienst ? 'Bijwerken' : 'Aanmaken'}</button>
                  </div>
                </form>
              </div>
            </div>
