...
export default function DienstenBeherenPage() {
  const [diensten, setDiensten] = useState<Dienst[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [editingDienst, setEditingDienst] = useState<Dienst | null>(null);
  const [formData, setFormData] = useState({ code: '', naam: '', beschrijving: '', kleur: '#3B82F6', actief: true });
  ...
  const openModal = (dienst?: Dienst) => {
    if (dienst) {
      ...
      setFormData({ code: '', naam: '', beschrijving: '', kleur: '#3B82F6', actief: true });
    }
    setError('');
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingDienst) {
        updateService(editingDienst.id, formData);
      } else {
  ...
  const handleDelete = async (dienst: Dienst) => {
    ...
    try {
      const check = canDeleteService(dienst.id);
      if (!check.canDelete) {
        alert(`Kan deze dienst niet verwijderen. ${check.reason}`);
        return;
      }
      removeService(dienst.id);
      loadDiensten();
    } catch (err: any) {
      ...
      <div className="max-w-6xl mx-auto">
        <div className="bg-white rounded-xl shadow-lg p-6 md:p-8">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-600">
                  Configureer diensttypes, kleurcodering en tijdsloten voor roosterplanning.
                </p>
              </div>
              <button
                onClick={() => openModal()}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {diensten.map((dienst) => (
              <div key={dienst.id} className="bg-white border border-gray-200 rounded-xl p-6 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold text-gray-900">{dienst.naam}</h3>
                ...
                <p className="text-gray-600 text-sm mb-4">{dienst.beschrijving}</p>
